...
class _ReelItemState extends State<ReelItem> {
  ...
  Future<void> _initMedia() async {
    if (_isInitialized || _retryCount >= _maxRetries || _isDisposing || widget.reel.mediaFile.isEmpty) {
    ...
    try {
      if (widget.reel.mediaType == 'video' && !_isAudioFile(widget.reel.mediaFile)) {
        print('DEBUG: Initializing VideoPlayerController for ${widget.reel.mediaFile} (software decoding: $_useSoftwareDecoding)');
        _videoController = VideoPlayerController.networkUrl(
          ...
          videoPlayerOptions: VideoPlayerOptions(
            ...
            webOptions: _useSoftwareDecoding ? const VideoPlayerWebOptions() : null,
          ),
        );
        ...
        print('DEBUG: VideoPlayerController initialized successfully for ${widget.reel.id}');
      } else {
        ...
        if (_isDisposing) return;
        print('DEBUG: AudioPlayer initialized successfully for ${widget.reel.id}');
      }
      ...
      if (mounted && !_isDisposing) {
        ...
        if (_retryCount < _maxRetries) {
          print('DEBUG: Retrying media initialization (attempt ${_retryCount + 1}/$_maxRetries, software decoding: $_useSoftwareDecoding)');
  ...
  void _play() {
    if (!_isInitialized || _errorMessage != null || _isDisposing || _videoController == null) return;
  ...
  void dispose() {
    ...
    if (_videoController != null && _videoController!.value.isInitialized) {
      ...
      _videoController = null; // Nullify to prevent reuse
    }
  ...
  void _showShareDialog(ReelsBloc reelsBloc, List<ShareTargetEntity> shareTargets) {
          ...
          builder: (BuildContext innerContext) {
            return StatefulBuilder(
              builder: (context, setState) {
                ...
                final filteredTargets = shareTargets.where((target) {
                  final displayName = target.type == 'group'
                    ...
                    children: [
                      ...
                      Expanded(
                        child: GridView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            final isSelected = selectedChatIds.contains(target.chatId);
                            return GestureDetector(
                              onTap: target.chatId != null
                                  ? () {
                                      setState(() {
                                        if (isSelected) {
                                          selectedChatIds.remove(target.chatId);
                                        } else {
                      ...
                      SizedBox(
                        ...
                        child: ElevatedButton(
                          onPressed: selectedChatIds.isNotEmpty
                              ? () {
                                  ...
                                  ScaffoldMessenger.of(innerContext).showSnackBar(
                                    SnackBar(
                                      content: Text('Reel shared with ${selectedChatIds.length} recipient(s)'),
                                    ),
                                  );
                                }
                              : null,
                          style: ElevatedButton.styleFrom(
                            backgroundColor: selectedChatIds.isNotEmpty ? Colors.blue : Colors.grey,
  ...
  Future<void> _shareExternally() async {
    final String message = 'Check out this reel: ${widget.reel.mediaFile}\nCaption: ${widget.reel.caption ?? 'No caption'}';
    try {
      await Share.share(
        message,
        subject: 'Shared Reel',
